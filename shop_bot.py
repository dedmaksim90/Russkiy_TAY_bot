...
async def check_rate_limit(user_id: int) -> tuple[bool, str]:
    """Проверка на флуд. Возвращает (пропускать_ли, сообщение_об_ошибке)"""
    now = time.time()
...
def validate_quantity(quantity_str: str, max_quantity: int = 1000) -> tuple[bool, int, str]:
    """Безопасная проверка количества товара"""
    try:
...
def sanitize_log_data(user_id: int) -> str:
    """Скрывает реальный user_id в логах"""
...
def anti_flood_handler(func):
    """Декоратор для защиты хендлеров от флуда"""
...
async def switch_to_admin_mode(message: types.Message):
    """Переключиться в режим администратора (без защиты от флуда для админа)"""
    if message.from_user.id != ADMIN_ID:
...
async def add_product_start(message: types.Message):
    """Начать добавление товара (только для админа)"""
    if message.from_user.id != ADMIN_ID:
        return
...
async def add_quantity_start(message: types.Message):
    """Начать пополнение остатков (только для админа)"""
    if message.from_user.id != ADMIN_ID:
        return
...
async def adjust_quantity_start(call: types.CallbackQuery, state: FSMContext):
    """Начало изменения остатков через кнопку (только для админа)"""
    if call.from_user.id != ADMIN_ID:
...
async def manage_products(message: types.Message):
    """Управление товарами (только для админа)"""
    if message.from_user.id != ADMIN_ID:
        return
...
async def edit_product_start(call: types.CallbackQuery):
    """Начать редактирование товара (только для админа)"""
    if call.from_user.id != ADMIN_ID:
        await call.answer("❌ Недостаточно прав", show_alert=True)
        return
...
async def edit_product_price_start(call: types.CallbackQuery, state: FSMContext):
    """Начать изменение цены товара (только для админа)"""
    if call.from_user.id != ADMIN_ID:
        await call.answer("❌ Недостаточно прав", show_alert=True)
        return
        ...
        if deleted_count > 0:
            ...
            try:
                await bot.send_message(ADMIN_ID,
                    f"🧹 <b>Автоматическое удаление данных</b>\n\nУдалено заказов: {deleted_count}\nСрок хранения: {days} дней",
                    parse_mode="HTML")
            except:
                pass
    except Exception as e:
...
async def callback_view_categories(call: types.CallbackQuery):
    """Обработчик кнопки К категориям"""
    await call.message.edit_reply_markup(None)
...
async def on_startup(dp):
    ...
    try:
        ...
        await bot.send_message(
            ...
            "🛡️ <b>Защита от флуда активирована</b>\n"
            "• Не более 1 сообщения в секунду\n"
            "• Автоблокировка на 5 минут при флуде\n"
